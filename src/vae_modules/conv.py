...
from vae_modules.ops import cast_tuple, video_to_image
...
class PaddedConv3D(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        if not is_causal:
            assert (
                self.time_kernel_size % 2 == 1
            ), "only odd time_kernel_size is supported for non-causal conv"
    ...
    def forward(self, x):
        if self.is_causal:
            ...
            padding = self.time_kernel_size // 2
            first_frame_pad = x[:, :, :1, :, :].repeat((1, 1, padding, 1, 1))
            last_frame_pad = x[:, :, -1:, :, :].repeat((1, 1, padding, 1, 1))
